...
const EditAdvertisement = () => {
        ...
        // extraFormData нужна, чтобы участвовать потом в качестве условия для второго useEffect'a, в случае если
        // что-то пойдет не так и не будет данных, то был такой пустой объект, который заблокирует срабатывание второго
        // useEffect'a, по скольку formData изначально не пустая, то прибегаем к такому способу, а если всё работает,
        // то formData совпадает с extraFormData


        const [extraFormData, setExtraFormData] = useState<any>(null)
        ...
        useEffect(() => {
            ...
            const fetchData = async () => {
                let getDocRef: DocumentReference<DocumentData>;
                getDocRef = doc(database, "advertisements", params.id as string);
                const getData = await getDoc(getDocRef)
                if (getData.exists()) {
                    setExtraFormData({...getData.data()} as any)
                    setFormData({
                        ...getData.data()
                    } as IFormData)
                    setIsLoading(false)
                    ...
                    navigate('/')
                    toast.error(`Advertisement doesn't exist`)
                }
    ...
    useEffect(() => {
        if (extraFormData && (extraFormData.userId !== auth.currentUser?.uid)) {
            toast.error(`You can't edit this advertisement`)
            navigate('/')
        }
    }, [extraFormData, navigate, auth.currentUser?.uid])
        ...
        const handleSubmit = async (e: FormEvent) => {
            ...
            await updateDoc(doc(database, 'advertisements', params.id as string), copyFormData)

            setIsLoading(false)
            toast.success('The advertisement was edited')
            navigate(`/category/${copyFormData.type}/${params.id}`)
        }
        ...
        return (
            <main className={'max-w-[768px] mx-auto'}>
                ...
                text-white font-semibold rounded-[10px] my-[15px] hover:bg-green-600 active:bg-green-700 transition duration-300 ease-in-out shadow-lg`}>Edit
                        an advertisement
                    </button>
                </form>
            </main>
